...
static constexpr char *TAG = (char*)"httpConfigTool";
...
void app_main(void){
    ...
    //ESP_LOGI(TAG, "stor_root.fist_child = %p", stor_root->get_first_child());
        ...
        if(action.compare("open")==0){
            ...
            if(file->exists()){
                ...
                if((ext.compare(".htm") == 0) || (ext.compare(".html") == 0)){
                    ...
                    storage_handle* settings_file = new storage_handle("/spiffs/settings.json");
                    global_settings* sett = new global_settings(settings_file);
                    sett->load_from_file();
